...
async function main() {
    ...
    const keypairFile = fs.readFileSync('scripts/keypair.json');
    ...
    await program.methods.setCustodyConfig({
        isStable: false,
        isVirtual: false,
        oracle: {
            ...
            maxPriceAgeSec: 60, // 1 minute
            oracleAuthority: wallet.publicKey,
        },
        pricing: {
            useEma: true,
            useUnrealizedPnlInAum: true,
            tradeSpreadLong: new BN(100),    // 1% spread for long positions
            tradeSpreadShort: new BN(100),   // 1% spread for short positions
            swapSpread: new BN(100),           // Keep swap spread at 1%
            minInitialLeverage: new BN(10000), // 1x min leverage
            maxInitialLeverage: new BN(20000), // 2x max initial leverage
            maxLeverage: new BN(30000),        // 3x max leverage
            maxPayoffMult: new BN(10000),      // 1x max payoff
            maxUtilization: new BN(9000),      // 90% max utilization
            maxPositionLockedUsd: new BN(1000000000), // $1000
            maxTotalLockedUsd: new BN(10000000000)    // $10000
        },
        permissions: {
            allowSwap: true,
    ...
    console.log("SOL custody configuration updated successfully!");
}
