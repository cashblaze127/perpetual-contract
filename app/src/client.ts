...
export class PerpetualsClient {
  ...
  constructor(connection: Connection, wallet: AnchorWallet) {
    ...
    if (!wallet || !wallet.publicKey) {
      throw new Error("Wallet with public key is required");
    }

    console.log("Initializing PerpetualsClient with:", {
    ...
    // Initialize AnchorProvider with proper commitment
    ...
    console.log("PerpetualsClient initialized successfully with program ID:", 
  ...
  addCustody = async (
    ...
    this.provider.wallet.signTransaction(tx);
  ...
  addLiquidity = async (
    ...
    console.log("Perpetuals", this.perpetuals.publicKey.toBase58());
    console.log("Pool", this.getPoolKey(poolName).toBase58());
    console.log("Custody", this.getCustodyKey(poolName, tokenMint).toBase58());

    let remAcc = await this.getCustodyMetas(poolName);
    console.log("remAcc", remAcc);
